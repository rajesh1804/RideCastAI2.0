...
#                     if bin_key in self.seen_bins[k]:
...
#         self.binner = BinningTransformer(max_bins_per_feature=20)
...
class ModelTrainer:
    ...
    def predict(self, x):
        return {
            "fare_pred": max(0.0, self.fare_model.predict_one(x)),
            "eta_pred": max(0.0, self.eta_model.predict_one(x))
        }

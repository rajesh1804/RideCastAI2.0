...
#         st.info("🧪 Drift Injection Active — the model is being challenged with noisy targets.")
...
#     st.subheader("📊 Rolling RMSE vs Baseline (Step View)")
#     fig2, ax2 = plt.subplots(1, 2, figsize=(14, 4))
#     ax2[0].step(range(len(model.fare_rmse_history)), model.fare_rmse_history, where="mid", label="Model RMSE", color='green')
#     ax2[0].step(range(len(model.fare_baseline_rmse)), model.fare_baseline_rmse, where="mid", label="Baseline RMSE", color='gray', linestyle="--")
#     ax2[0].set_title("Fare RMSE Comparison")
#     ax2[0].legend()

#     ax2[1].step(range(len(model.eta_rmse_history)), model.eta_rmse_history, where="mid", label="Model RMSE", color='purple')
#     ax2[1].step(range(len(model.eta_baseline_rmse)), model.eta_baseline_rmse, where="mid", label="Baseline RMSE", color='gray', linestyle="--")
...
with tab3:
    ...
    st.info("📘 Shows model inference speed.")
    stats = latency.summary()
    st.metric("Min Latency (ms)", stats['min'])
    st.metric("Max Latency (ms)", stats['max'])
    st.metric("Avg Latency (ms)", stats['avg'])
...
with tab4:
    ...
    st.caption("Built by Rajesh Marudhachalam — top 0.001% ML Engineer candidate.")
